...
- **Visão em grafo (nós conectados)**: não existe no catálogo shadcn nem nos 135+ registries comunitários pesquisados via Shoogle — só há gráfico de barra/linha/pizza (Recharts) e `contribution-graph` (heatmap tipo GitHub), que resolvem outro problema. Caminho recomendado para quando for construído: `@xyflow/react` (React Flow), a mesma base por trás dos componentes `node`/`edge` do registry `@ai-elements` do AI SDK da Vercel — desenhado justamente para diagramas de agentes/skills. Alternativa mais leve: `react-force-graph`. Nenhuma dependência disso foi instalada ainda.
